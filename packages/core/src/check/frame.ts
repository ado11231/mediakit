 ...
 *
 * These cannot live in `check`, which does not render and so has no geometry to read. They
 * report from `render`, where the author is looking, and gate in `export`, which is the last
 * thing between a spec and an upload.
 */
...
export const checkFrame = (input: RenderedFrameCheck): Violation[] => {
  ...
  return checkOverflow(frame, textBoxes, context);
};
